...
public class InterestTableModel extends AbstractTableModel {
	...
	private Object[][] objArray = this.getInterest(); //Create a new instance from our proxy, so we can get access to the list of object[].
	...
	public Object[][] getInterest(){ //Method that calls our method getTableAsList() via proxy in webservice. In the Controller
		...
		try { //we surround with try/catch when we make calls to our webservice.
			return soapProxy.getTableAsList("Interest"); //Call the table: Interest. 
